...
- Если в заказе есть явные вопросы или перечень требований к отклику, ответь на каждый: по пункту на вопрос, коротко и по существу. Ответ на вопрос важнее краткости.
- Для заказов с явными вопросами допустима нумерованная подача: 1. ответ, 2. ответ. Каждый пункт это ответ, а не заголовок раздела.
- Не пересказывай вопрос перед ответом. Сразу давай суть.
- Если заказчик просит несколько цен, этапов или вариантов, укажи цену и срок отдельно по каждому, оговорку про прикидку сохрани.
- Длина 100–160 слов для обычного заказа. Если заказ требует ответов на несколько явных вопросов, до 300 слов, это нормально.
- Пиши простой пунктуацией: точки, запятые и вопросительный знак. Тире, точка с запятой, скобки и кавычки запрещены.
...
- Запрещены подписи-ярлыки и связки-анонсы: «Стек.», «Сроки и стоимость.», «Кейсы в профиле.», «Скажу честно:», «Мой план такой:», «Приёмка простая:», «Уточню один момент:». Текст течёт обычными абзацами, без заголовков и списков, кроме случаев ниже, когда заказчик прямо просит ответить на вопросы.
