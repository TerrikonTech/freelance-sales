...
оценивает релевантность и цену, готовит отклики и сообщения. Отклики FL.ru
по подходящим заказам (грейд medium/large или цена ≥ 30 000 ₽, заказ свежее
40 минут) отправляются автоматически серверным сервисом `fl-sender` — без
ручного подтверждения, с уведомлением в Telegram. Всё остальное — сообщения
клиентам, follow-up — только после явной команды владельца.

## Документация

- [Архитектура и потоки данных](docs/ARCHITECTURE.md) — компоненты, Mermaid-схемы,
  жизненный цикл заказа, чат, документы, AI-очередь и модель данных.
- [Автоотправка откликов FL.ru](docs/FL_AUTOSEND.md) — сервис fl-sender:
  отбор кандидатов, механика формы, cookies, анти-блок дизайн, эксплуатация
  и устранение неполадок.
- [Telegram-бот и агент общения](docs/TELEGRAM_AGENT.md) — команды владельца,
  память, выбор клиента, черновики, миссии, Business API, личная сессия и
  ограничения автономности.
- [Эксплуатация](docs/OPERATIONS.md) — установка, обновление, здоровье,
  журналы, резервное копирование, восстановление и безопасный откат.
- [Конфигурация](docs/CONFIGURATION.md) — полный перечень runtime-переменных,
  безопасные значения по умолчанию и границы между `.env`, Compose и секретами.
- [Тестирование](docs/TESTING.md) — модульные тесты, изолированный «Полигон»,
  «Лаборатория общения» и проверки, которые не расходуют отклики FL.ru.
- [Генерация откликов](docs/PROPOSALS.md) — структура, технологический фит,
  доказательства, коммерческие сигналы и финальный валидатор.
- [Карта репозитория](docs/REPOSITORY_MAP.md) — назначение каталогов и файлов,
  а также перечень данных, которые намеренно не попадают в Git.
- [Реализация исследования](docs/RESEARCH_ROADMAP.md) — воронка, follow-up,
  прогрессивная автономность, память и обязательные safety gates.

## Что является полной версией системы

Этот репозиторий содержит весь воспроизводимый исходный код приложения:
NestJS API и worker, React PWA, SQL-схему, AI-брокеры Hermes/Codex, FL.ru и
Telegram-интеграции, безопасный мост личной Telegram-сессии, Docker/systemd
конфигурацию, скрипты миграции и проверки, библиотеку публичных кейсов и
нечувствительные portfolio-assets.

В Git намеренно **не входят** эксплуатационные данные и секреты: `.env`, токены,
cookies FL.ru, Telegram-сессии, PostgreSQL/Redis volumes, клиентские документы,
runtime-очереди, логи, дампы и локальные бэкапы. Они не являются исходным кодом;
их публикация сделала бы репозиторий небезопасным. Полное восстановление поэтому
требует двух частей: этого репозитория и отдельного зашифрованного бэкапа данных.

Исходники в Git являются единственной публикуемой версией кода. Production
checkout может дополнительно содержать `node_modules`, `dist`, `.env`, runtime и
данные, но функциональные файлы приложения должны совпадать с Git commit, из
которого собран образ. Отдельная ручная «копия для публикации» источником истины
не является.
...
Из Telegram владельца доступны два дополнительных on-demand сценария:

- `собери ТЗ` — агент готовит один следующий вопрос выбранному клиенту,
  показывает его владельцу и ждёт отдельную команду `отправь`;
- `сгенерируй 4 концепции дизайна по https://…` — система безопасно читает
  только публичную страницу, готовит четыре предварительных эскиза и показывает
  их владельцу. Медиагруппа и подпись клиенту также уходят только после
  отдельной команды `отправь`.

Для изображений в настройках дашборда нужен отдельный OpenAI API key. Он
хранится зашифрованно в таблице settings. По умолчанию используется
`gpt-image-2`, landscape 1536×1024, quality `low`, максимум четыре варианта;
платная генерация не повторяется автоматически после обрыва или неизвестного
результата. Публичные ссылки на эскизы имеют HMAC-подпись и срок действия.
...
- `STRICT_APPROVAL=true` принудительно оставляет ручной режим даже при старой
  ...
  Telegram-черновики и инициативные сообщения бота, кроме live-ответов по
  адресной миссии владельца с сохранением stop-гейтов и лимита ходов.
