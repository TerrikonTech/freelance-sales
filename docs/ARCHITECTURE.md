...
- `TELEGRAM_ON_DEMAND_ONLY=true` не позволяет входящему Telegram-сообщению само
  запустить генерацию или инициативный ответ.
...
## Что реализовано, но намеренно не автономно

`lead_missions` и команды «общайся с …» существуют, но при production-настройке
`TELEGRAM_ON_DEMAND_ONLY=true` новое Telegram-сообщение клиента само не создаёт
следующий ответ. Миссия сохраняется и видна владельцу, однако сквозной
автономный диалог не включён. Follow-up по расписанию создаёт черновик и
уведомление владельцу, но сам клиенту не отправляется. К существующим данным
добавлены `followup_schedule`, `autonomy_class_stats`, `autonomy_feedback`,
`agent_eval_results`, `quality_cases` и `lead_episodes`. Это сознательная
граница безопасности, а не обещание фоновой автономности.
