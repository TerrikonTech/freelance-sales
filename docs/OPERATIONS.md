...
`auto` сначала проверяет Hermes и запускает Compose `ai-broker`, затем при
неуспехе использует host-side Codex fallback. Перед выбором `deploy.sh`
останавливает оба возможных потребителя; оба варианта обслуживают одну таблицу
`ai_tasks`, и одновременный запуск двух потребителей недопустим.
