...
Коммерческий блок обязателен: точно повтори в content цену price_rub и срок duration_days. Если availability_configured=true, дословно используй availability. Если availability_configured=false, дату старта не выдумывай и не создавай заглушку «после согласования объёма и доступов»: интерфейс отдельно предупредит владельца. Это нужно, даже если FL.ru также покажет цифры в отдельных полях. mention_price_in_body верни true. Если client_name подтверждён, коротко обратись по имени; логин не превращай в имя.
...
Варианты должны отличаться логикой, а не перестановкой одинаковых блоков. Первую содержательную фразу каждого начни с opening_seed дословно и сразу продолжи непереносимой деталью заказа. Для трёх candidates используй три разных hook_pattern; один из них обязан совпасть с variation_plan.required_hook_pattern. В каждом: минимум две детали заказа; минимум два естественных обращения «у вас/вам/ваш»; хотя бы одно действие в форме «я соберу/покажу/проверю»; 1–3 уместные разговорные связки; микро-план 2–3 шага; измеримый критерий как сцена с сотрудником или пользователем; ровно одно доказательство; точные commercial_terms.price_rub и commercial_terms.duration_days; условие старта только при availability_configured=true; ровно один лёгкий вопрос в финале. variation_plan.required_acceptance_text — подсказка против повторов, не канцелярская обязаловка. Цену и срок напиши живой фразой, сумму — с разрядами; «Стоимость — N, срок — M» запрещено. Хотя бы один вариант должен обходиться без стажа и Яндекса. Если portfolio непуст, каждый вариант использует ровно один релевантный кейс с точным URL. Кейс строй только из заполненных полей case_card: «я делал → конкретная деталь → связь с задачей → ссылка». Пустые поля нельзя домысливать. Если technology_fit.risk=elevated и используется соседний кейс, прямо скажи человеческими словами, что тот проект был не на требуемой CMS/технологии и стек другой; протокольное «не служит подтверждением» запрещено. Не заявляй неподтверждённый опыт и не маскируй его архитектурными терминами. Не превращай текст в формулу «что сделаю + 6 лет + вопрос».

Первые 150–200 символов — самое дорогое место. Не трать их на «Здравствуйте», «я разработчик», стаж или заверение, что задание прочитано. Начни с проблемы, наблюдения, результата или близкого доказательства для клиента. Первое содержательное предложение — не более 24 слов; сложную мысль разбей на две фразы. Короткое обращение по подтверждённому имени допустимо отдельной строкой. В первой содержательной фразе нужен якорь, который нельзя без изменений вставить в случайный заказ. Используй вторую unique_signal дальше по тексту, не перечисляя бриф. Близкий кейс называй только при реальном совпадении продукта, интерфейса или архитектуры и объясни сходство. Не пиши безликое «в портфолио есть платформа». Не выдумывай кейсы, скриншоты, клиентов, метрики, даты старта или сделанную работу.
...
Не выбирай вариант, состоящий в основном из уточняющего вопроса. Итог начинает первую содержательную фразу с opening_seed и сразу привязывает её к непереносимой детали заказа. variation_plan.required_acceptance_text используй как подсказку против повторов, а не как дословную обязаловку. Проверь микро-план 2–3 шагов и измеримую сцену приёмки, а не «всё работает». Для premium добавь управленческую зрелость: этапность, QA, прозрачность, откат или обратимость. Для compact не раздувай простую правку искусственными рисками. Бесплатный тестовый экран и пробная реализация запрещены, если заказчик их не просил.

Финал должен продавать только следующий шаг и содержать ровно один простой вопрос или бинарный выбор. Проверь каждое утверждение о кейсе по seller/portfolio. Если portfolio непуст, итог использует ровно один релевантный кейс и формулу «я делал → одна живая деталь строго из заполненных полей case_card → связь с задачей → точный URL»; других ссылок нет. Абстрактное «проектировал роли и кабинет» без проверяемой детали не проходит. Если поле карточки пустое, факт из него выдумывать запрещено. Если portfolio пуст, не выдумывай кейс и опирайся на подтверждённый опыт или прозрачность первого этапа. Отдельно сверь technology_fit: verifiedEvidence разрешает одно конкретное доказательство по требуемому стеку; risk=elevated запрещает утверждать опыт с этой CMS/технологией. Если при risk=elevated оставляешь соседний кейс, напиши естественно и прямо: «Скажу честно: тот проект был не на [технологии], стек другой». Не используй канцелярское «не служит подтверждением опыта». Архитектурные термины не являются доказательством. 6 лет и Яндекс не вставляй механически. Не придумывай выполненную работу, клиентов, цифры, скриншоты, доступность или обещание показать отсутствующий материал.

Проверь обязательные коммерческие сигналы в content: точную цену commercial_terms.price_rub и точный срок commercial_terms.duration_days. При availability_configured=true также обязательна точная commercial_terms.availability; при false доступность нельзя выдумывать или заменять системной заглушкой. Не убирай подтверждённые сигналы как дубль полей FL.ru. Если greeting_required=true, проверь обращение по client_name. Сравни с recent_drafts и отвергни повтор той же длины, синтаксиса, захода, доказательства или типа финала. Каркас «сделаю X. Я 6 лет в разработке. Вопрос?» считается шаблоном и должен быть переписан целиком.

Готовый content одновременно соблюдает minWords/maxWords и minChars/maxChars из proposal_profile, имеет 2–5 содержательных абзацев разной длины, микро-план, живую сцену приёмки, один кейс, цену, срок, подтверждённый старт при его наличии и один вопрос. Жёсткий потолок 300 слов. Целевой бёрстинесс ≥0,60, жёсткий провал <0,55. Обязательно: одна содержательная короткая фраза на 2–5 слов, короткое продолжение после фразы от 25 слов, разная конструкция соседних предложений, хотя бы одно действие через «я сделаю/покажу/проверю», минимум два обращения к клиенту. Стена текста, пустые короткие филлеры, «1) 2) 3)» внутри предложения, правило трёх, безличные старты, номинализационный канцелярит, симметричные абзацы, подзаголовки, эмодзи, пафос, давление, пересказ заказа и формулы «лучше X, иначе Y»/«не просто X, а Y» запрещены. human_score ниже 85 ставь любому тексту, который звучит написанным ИИ; sales_score ниже 85 — если нет ясной причины продолжить разговор или снижения риска; specificity_score ниже 90 — если нет двух деталей конкретного заказа; factual_score ниже 100 — если есть неподтверждённый факт. До возврата перепиши так, чтобы factual_score был 100, human_score и sales_score были не ниже 85, specificity_score не ниже 90. Верни только JSON по схеме.""",
