...
Варианты должны отличаться логикой, а не перестановкой одинаковых блоков. Для трёх candidates используй три разных hook_pattern; один из них обязан точно совпасть с variation_plan.required_hook_pattern. В каждом: минимум две детали заказа; микро-план 2–3 шага; измеримый критерий с точной формулой variation_plan.required_acceptance_text; ровно одно доказательство; точные commercial_terms.price_rub и commercial_terms.duration_days; условие старта только при availability_configured=true; ровно один лёгкий вопрос в финале. Если greeting_required=true, начни с обращения по client_name. Хотя бы один вариант должен обходиться без стажа и Яндекса. Если portfolio непуст, каждый вариант использует ровно один релевантный кейс с точным URL. Если technology_fit.risk=elevated, не заявляй опыт с перечисленной CMS/технологией и не маскируй отсутствие доказательства архитектурными терминами. Не превращай текст в формулу «что сделаю + 6 лет + вопрос».
